...
[![GitHub Stars](https://img.shields.io/github/stars/kongweihao/wsc?style=social)](https://github.com/kongweihao/wsc)
...
1. 克隆代码仓库
   ...
   ```
2. 部署到 Web 服务
   - Nginx/Apache：将项目放置到服务根目录的 `wsc/` 文件夹下
   - 本地测试：执行以下命令启动简易静态服务
     ```bash
     python -m http.server 8080
     ```
3. 访问系统
   - 服务器部署：`http://<你的服务器IP>/wsc/index.html`
   - 本地测试：`http://localhost:8080/wsc/index.html`
4. 免登录演示模式（跳过生产环境登录校验）
   ```
   http://<你的服务器IP>/wsc/index.html?developmode=1
   ```
...
### 地图模式
- 2D/3D 切换：通过右下角 Speed Dial 控制台一键切换，3D 模式支持纹理切换（地形/夜光等）、自动旋转（速度 0.2~20.0× 可调）
...
| v3.5   | 2026-01-15 | 接入海缆故障系统，新增登录验证，完善数据安全管控                         |
| v4.0   | 2026-01-16 | 角色权限管理、操作日志、多用户协作                                     |
| v5.0   | 2026-01-20 | 高分辨率瓦片地图、精细化地理信息展示                                    |
| v5.0   | 2026-01-21 | 瓦片打点与全局深度优化                                    |
| v6.0   | 2026-01-23 | 全局 i18n 改造与紧凑体验收敛                                     |
| v6.5   | 2026-01-23 | 瓦片地图 7 级升级                                     |
...
## 🧭 战略规划
本项目作者基于此项目的技术能力与行业积累，整理了一份完整的创业与财务自由战略规划文档，涵盖：
- 真正具备长期壁垒的方向分析
- 最大认知误区与短板直指
- 从上班族到独立创造者再到可扩张事业的最优路径
- 第一个最小可行项目（MVP）设计
- 单人到体系化的增长闭环
- 3 年落地战略规划
